...
bool Scene::Init()
{
	...
	GameObject* test_camera_object = new GameObject();
...
void Scene::OutputDebugMessage()
{
	...
	for each (GameObject* game_object in game_objects_)
	{
		std::cout << std::endl << "GameObject:" << std::endl;
		std::cout << "\tpos: " << game_object->GetPosition()->x << ", " << game_object->GetPosition()->y << ", " << game_object->GetPosition()->z << std::endl;
