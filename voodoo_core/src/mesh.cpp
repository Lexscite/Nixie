...
unsigned long Mesh::GetVertexCount() { return v_count_; }

unsigned long Mesh::GetIndexCount() { return i_count_; }

std::vector<VertexPTN> Mesh::GetVertices() { return v_; }

std::vector<unsigned long> Mesh::GetIndices() { return i_; }

ID3D11Buffer* Mesh::GetVertexBuffer() { return vertex_buffer_; }

ID3D11Buffer* Mesh::GetIndexBuffer() { return index_buffer_; }
...
bool Mesh::CreateVertexBuffer() {
  ...
  HRESULT hr = device_->CreateBuffer(&vertex_buffer_desc, &vertex_buffer_data,
                                     &vertex_buffer_);
  if (FAILED(hr)) {
    return false;
  }
...
bool Mesh::CreateIndexBuffer() {
  ...
  HRESULT hr =
      device_->CreateBuffer(&index_buffer_desc, &index_data, &index_buffer_);
  if (FAILED(hr)) {
    return false;
  }
